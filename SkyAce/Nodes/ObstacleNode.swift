...
final class ObstacleNode: SKNode {
    ...
    private func buildPillars(sceneSize: CGSize) {
        ...
        if bottomHeight > 0 {
            let bottom = makePillar(width: pillarWidth, height: bottomHeight)
        ...
        if topHeight > 0 {
            let top = makePillar(width: pillarWidth, height: topHeight)
            top.position = CGPoint(x: 0, y: topY0 + topHeight / 2)
            addChild(top)
        }
    }

    private func makePillar(width: CGFloat, height: CGFloat) -> SKNode {
        // Stack of overlapping cloud circles — reads as a fluffy cloud pillar.
